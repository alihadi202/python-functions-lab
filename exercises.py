...
def simple_interest(pri , rate , time):
    return (pri*(rate/100)*time)
...
def convert_temperature(temp , X ):
    if X == "C":
        return (temp * 9/5) + 32
    elif X == "F":
        return (temp - 32) * 5/9
...
def basicCalculator(x , y , s ):
    z = x*y if s=="multiply" else x+y if s=="add" else x-y if s=="subtract"else x/y
    return z
